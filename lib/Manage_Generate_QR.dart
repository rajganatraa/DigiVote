...
// String test;
class managegenerateqrpage extends StatefulWidget {
  // final String ID;
  const managegenerateqrpage({required this.ID});
  final String ID;

  @override
  State<StatefulWidget> createState() => managegenerateqrpageState();
}

class managegenerateqrpageState extends State<managegenerateqrpage> {
  ...
  void captureScreenshot() async {//made change in manifest and pinfo file
    ...
    if(byteData != null){
      ...
      // print(saveimage);
      showStatusMessage("QR Image saved to Gallery", Colors.blue); //saveimage.toString()
    }else {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('QR Code',style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
        actions: <Widget>[
          IconButton(
